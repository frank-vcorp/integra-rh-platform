...
export const appRouter = router({
  ...
  auth: router({
    // Debe requerir autenticación; de lo contrario, responde 200 con user null
  ...
  processes: router({
    ...
    create: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // Generar token de acceso para el cliente y enviar email
        try {
          const { createClientAccessToken, getClientAccessUrl } = await import('./auth/clientTokens');
          const client = await db.getClientById(input.clienteId);
          const candidate = await db.getCandidateById(input.candidatoId);
          
          if (client && client.email && candidate) {
            // Generar token (válido por 30 días)
            const token = await createClientAccessToken(input.clienteId, 30);
            
            // Obtener URL de acceso (usar URL base de la aplicación)
            const baseUrl = process.env.VITE_APP_URL || 'http://localhost:3000';
            const accessUrl = getClientAccessUrl(token, baseUrl);
          ...
          console.error('Error al generar token o enviar email:', error);
          // No fallar la creación del proceso si falla el email
        }
  ...
  // ==========================================================================
  // ACCESO DE CLIENTES (Token Único)
  // ==========================================================================
  ...
  clientAccess: router({
    validateToken: publicProcedure
      ...
      .query(async ({ input }) => {
        console.log('[validateToken] Recibido token:', input.token.substring(0, 20) + '...');
        const { validateClientToken } = await import('./auth/clientTokens');
        const client = await validateClientToken(input.token);
        console.log('[validateToken] Cliente encontrado:', client ? `ID: ${client.id}, Nombre: ${client.nombreEmpresa}` : 'null');
        
        if (!client) {
          console.log('[validateToken] Token inválido o expirado');
          return { valid: false, clientId: null };
        }
        
        console.log('[validateToken] Token válido, clientId:', client.id);
