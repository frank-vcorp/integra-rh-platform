...
/**
 * FIX-20260217-01: Cloud Function para sincronizar plazas (clientSiteId) en procesos históricos
 * Endpoint: POST /migrateProcessSites
 * Autenticación: Requiere auth header
 * 
 * Uso:
 * curl -X POST https://tu-proyecto.cloudfunctions.net/migrateProcessSites \
 *   -H "Authorization: Bearer ${TOKEN}"
 */
exports.migrateProcessSites = functions.https.onRequest(async (req, res) => {
  // Validar método
  if (req.method !== "POST") {
  ...
  try {
    ...
    result.affectedRows = updateResult.affectedRows || 0;
    logger.info(`[Migration] Updated ${result.affectedRows} processes with sites`);

    // 2. Contar procesos que aún no tienen plaza
    const countQuery = `SELECT COUNT(*) as count FROM processes WHERE clientSiteId IS NULL`;
    const [countResult] = await connection.execute(countQuery);
    result.processesWithoutSites = countResult[0]?.count || 0;

    // 3. Log de procesos sin plaza
    if (result.processesWithoutSites > 0) {
      ...
      logger.info(`[Migration] Found ${result.processesWithoutSites} processes without sites:`, orphanResults);
    }
    ...
    if (connection) {
      try {
        ...
        logger.error("[Migration] Error closing connection:", closeError);
      }
    }
  }
});
 ...
 * Endpoint: GET /validateProcessSites
 * Útil para health checks
 */
exports.validateProcessSites = functions.https.onRequest(async (req, res) => {
  if (req.method !== "GET") {
    return res.status(405).json({ error: "Method not allowed" });
  }
  ...
  try {
    ...
    const processesWithoutSites = countResult[0]?.count || 0;

    // Contar total de procesos
    const [totalResult] = await connection.execute(
      `SELECT COUNT(*) as count FROM processes`
    );
    const totalProcesses = totalResult[0]?.count || 0;
